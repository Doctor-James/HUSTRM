...
namespace ly
{
    armorThread::armorThread(armor_param config, armor_param large, cam_param cam_config, lightBarThread *lightBar, serialPortReadThread *serialPortRead, serialPortWriteThread *serialPortWrite, cameraThread *camera) : lightBar_(lightBar), serialPortRead_(serialPortRead), serialPortWrite_(serialPortWrite), camera_(camera)
    {
        score_ = new score();
    ...
    void armorThread::process()
    {
        ...
        if (lightBar.size() > 1)                                                  //有两个以上灯条
        {
            ...
            //counter_.countEnd();
            //std::cout << "armor cost time" << counter_.getTimeMs() << std::endl;
            //              //大小装甲板判断
    ...
    //根据检测到的装甲板的点，来截取ROI（进行透视变换），得到较为规整的装甲板ROI
    cv::Mat armorThread::getArmorRoi(armorNode bestArmor)
    {
        ...
        return dstImage(cv::Rect(x_, y_, width_, height_));
    }
    void armorThread::updateArmor()
    {
        std::priority_queue<armorNode> armor_copy;
        armor_copy = armor_;
        {
